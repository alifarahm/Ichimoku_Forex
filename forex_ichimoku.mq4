...
// Kijunsen/Price Cross
     ...
     if (Close[0]<kijunSen) KijenCloseBear = True;
     else KijenCloseBear = False;
     if (Close[0]>kijunSen)  KijenCloseBull = True;
   ...
   if(((Close[1]<SenkouSpanALatestByPrice) && (KumoBull) && (Close[0]>SenkouSpanA) && (Close[2]<SenkouSpanA2) &&(TenkanStateBull))|| ((TenkanStateBull)&&(Close[1]<SenkouSpanBLatestByPrice) && (KumoBear) && (Close[0]>SenkouSpanB) && (Close[2]<SenkouSpanB2))) KumoBullCross=True;
   ...
   if ((TenkenKigunStateBull) && ((tenkanSen-kijunSen>0.004)) && (KijunZeroTrend) && (TenkenCrossBear)&&(KumoConfirmation)) St3= True;
   else St3=False;
   
   if ((TenkenKigunStateBear) && ((kijunSen-tenkanSen>0.004)) && (KijunZeroTrend) && (TenkenCrossBull)&&(KumoConfirmation)) St4= True;
   else St4=False;
      ...
      if(OrderType() <= OP_SELL &&  OrderSymbol() == Symbol() && OrderMagicNumber() == MagicNumber) {
         ...
         if(OrderType() == OP_BUY) {
           ...
           // if ((KijunCrossBull)) Order = SIGNAL_CLOSESELL; // Rule to EXIT a Short trade
            ...
            if (STR=11 && KijunCrossBull) Order = SIGNAL_CLOSESELL; 
            else if (STR=12 && ((KijunCrossBull)||((Close[0]>SenkouSpanA)&&(Close[0]>SenkouSpanB))&& Close[0]>tenkanSen)) Order = SIGNAL_CLOSESELL;
            else if ((STR=13) && (KijunCrossBull)) Order = SIGNAL_CLOSESELL;
            else if ((STR=14) && ((KijunCrossBull)&& (Close[1]>kijunSen))) Order = SIGNAL_CLOSESELL;
   ...
   //+------------------------------------------------------------------+
    ...
    if (New_Bar()) if ((( TenkenCrossKingunBull)&&(TenkenTrendBull || TenkenTrendZero) ))  {Order = SIGNAL_BUY; STR=1;} //  (tenkensen Cross Kijunsen)
    if (New_Bar()) if (KumoBullCross) {Order = SIGNAL_BUY; STR=2;}                                                     ///  (Close Cross Kumo)
    if (New_Bar()) if (St4) {Order = SIGNAL_BUY;STR=3;}                                                                ////  (Close between Kijunsen and Tenkensen)
    if (New_Bar()) if (St5) {Order = SIGNAL_BUY;STR=4;}                                                                ///// (Close Cross tenkensen and kijunsen)
  
 
   if (New_Bar()) if (( TenkenCrossKingunBear)&&(TenkenTrendBear) && (New_Bar()) ) {Order = SIGNAL_SELL;STR=11;} //  (tenkensen Cross Kijunsen)
   if (New_Bar()) if (KumoBearCross) {Order = SIGNAL_SELL;STR=12;}                               /// (Close Cross Kumo)
   if (New_Bar()) if (St3) {Order = SIGNAL_SELL;STR=13;}                                         ////  (Close between Kijunsen and Tenkensen)
   //if (St6) {Order = SIGNAL_SELL; STR=14;}                                        ///// (Close Cross tenkensen and kijunsen)
